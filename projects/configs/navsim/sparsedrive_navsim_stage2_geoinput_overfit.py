# Phase-3 overfit gate (NAVSIM_SPARSEDRIVE_PORT.md): detection + map +
# motion + V6 geometric planner on the same frozen manifest of the first
# eight sorted navmini tokens as the Phase-1/2 gates. Gate: total planning
# (and detection/map) losses fall by >= 95% from their first-20-step means.
#
# Mirrors sparsedrive_navsim_stage1_maps_overfit.py exactly (no photometric
# jitter, no grid mask, no flips/rotation, deterministic resize/crop,
# sequences_split_num="all"), plus the motion/planning GT keys. Initializes
# from the Phase-1 stage-1 overfit checkpoint (injected by the Lilypad
# entrypoint via load_from).
